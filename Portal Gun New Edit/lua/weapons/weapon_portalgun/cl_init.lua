...
function SWEP:ViewModelDrawn(vm)
	...
	local cBlu = self.Primary.Color
	local cOrg = self.Secondary.Color
...
--local cBlu = Color(64,160,255,120)
--local cOrg = Color(255,160,32,120)
...
function SWEP:DrawHUD()
	...
	local portal_convar = GetConVar("portal_portalonly"):GetInt()
	
	local firemode = self.Weapon:GetNWInt( "Portal:FireMode", 0 )
	local cannotFireBlue = (bit.band(firemode, TYPE_BLUE) == TYPE_BLUE) or (portal_convar == 2)
	local cannotFireOrange = (bit.band(firemode, TYPE_ORANGE) == TYPE_ORANGE) or (portal_convar == 1)
	
	if (cannotFireBlue and cannotFireOrange) or (portal_convar >= 3) then return end -- Dud portal gun, don't draw any crosshair.
	
	local drawmaterial
	if validMat and (!cannotFireBlue and validBlu or cannotFireBlue and validRed) then
		surface.SetMaterial( Material("vgui/portalgun/leftFull.png"))
	else
		surface.SetMaterial( Material("vgui/portalgun/leftEmpty.png"))
	end
	if cannotFireBlue then
