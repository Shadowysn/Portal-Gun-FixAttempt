...
function SWEP:ViewModelDrawn(vm)
	...
	for k, name in pairs( self.VElements ) do
		...
		if (v.type == "Sprite" and sprite) then
			...
			if last == TYPE_Yellow then
			col = Color(255,242,0,255)
			elseif last == TYPE_Red then
...
local cBlu = Color(255,242,0,120)
local cOrg = Color(255,0,0,120)
...
function SWEP:DrawHUD()
	...
	local cRit = (self:GetNetworkedBool("OnlyYellow") and cBlu or cOrg)
	...
	if hEnt != nil && hEnt:IsValid() && hEnt:GetClass() == "prop_portal_pbody" then
		if hEnt:GetNWInt("Potal:PortalType",TYPE_Yellow) == TYPE_Yellow then
			validRed = false
		else
			validBlu = false
		end
...
killicon.Add( "prop_portal_pbody", "hud/killicon_portals", Color( 255, 48, 0, 255 ) )