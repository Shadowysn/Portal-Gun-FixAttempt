...
if ( CLIENT ) then
        ...
        SWEP.PrintName          = "P-Body's Portal Gun"
...
SWEP.WorldModel                 = "models/weapons/portalgun/w_portalgun_orange.mdl"
...
SWEP.HasRedPortal = false
SWEP.HasYellowPortal = false
...
function SWEP:ShootPortal( type )
	...
	local RedPortalEnt = owner:GetNWEntity( "Portal:Red", nil )
	local YellowPortalEnt = owner:GetNWEntity( "Portal:Yellow", nil )
   
	local EntToUse = type == TYPE_Yellow and YellowPortalEnt or RedPortalEnt
	local OtherEnt = type == TYPE_Yellow and RedPortalEnt or YellowPortalEnt
					  ...
					  timer.Simple( hitDelay - .05, function()
							if ball and ball:IsValid() then 
								...
								if !IsValid( EntToUse ) then
										...
										if type == TYPE_Yellow then
									   
												owner:SetNWEntity( "Portal:Yellow", Portal )
												Portal:SetNetworkedBool("Yellow",true,true)
											   
										else
									   
												owner:SetNWEntity( "Portal:Red", Portal )
												Portal:SetNetworkedBool("Yellow",false,true)
                        ...
                        ent:SetKeyValue( "effect_name", "portal_" .. type .. "_badsurface_pbody")
...
function SWEP:CleanPortals()
        ...
        local Yellowportal = self.Owner:GetNWEntity( "Portal:Yellow" )
        local Redportal = self.Owner:GetNWEntity( "Portal:Red" )
        local cleaned = false
       
        for k,v in ipairs( ents.FindByClass( "prop_portal_pbody" ) ) do
       
                if v == Yellowportal or v == Redportal and v.CleanMeUp then
...
function SWEP:CheckExisting()
	if Yellowportal != nil && Yellowportal != nil then return end
	for _,v in pairs(ents.FindByClass("prop_portal_pbody")) do
		local own = v.Ownr
		if v != nil && own == self.Owner then
			if v.type == TYPE_Yellow && self.Yellowportal == nil then
				self.Yellowportal = v
			elseif v.type == TYPE_Red && self.Redportal == nil then
				self.Redportal = v
			end
		end
	end
