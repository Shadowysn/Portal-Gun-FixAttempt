...
ENT.PortalColorBlue		= Color( 0, 80, 255, 255 )
ENT.PortalColorOrange	= Color( 255, 107, 0, 255 )
...
function ENT:RegisterToGun(wep)
	if !IsValid(wep) or !wep:IsWeapon() then return end
	
	if string.find(wep:GetClass(), "atlas", 1) != nil then
		self.PortalColorBlue = Color( 0, 225, 255, 255 )
		self.PortalColorOrange = Color( 0, 0, 255, 255 )
	end
end
...
function ENT:GetOther()
	return self:GetNWEntity("Potal:Other", nil)
end
	...
	for i=0,54 do
		...
		if openspace then
			...
			-- net.Start("DebugOverlay_Cross")
				-- net.WriteVector(self:LocalToWorld(pos+offset))
				-- net.WriteBool(true)
			-- net.Broadcast()
			offset.z = i
			break
