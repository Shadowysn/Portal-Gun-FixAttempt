...
function ENT:BootPlayer()
	...
	for k,p in pairs(player.GetAll()) do 
		if p.InPortal and (p.InPortal:EntIndex() == self:EntIndex()) then
			...
			net.Start( "Portal:ObjectLeftPortal" )
			net.WriteEntity( p )
			net.Broadcast()
		end
	end
...
function ENT:MoveToNewPos(pos,newang) --Called by the swep, used if a player already has a portal out.
	...
	if IsValid(self.VacuumEffect) then
		self.VacuumEffect:SetPos(effectpos)
	end
	if IsValid(self.EdgeEffect) then
...
function ENT:MakeClone(ent)
	...
	net.Start("Portal:ObjectInPortal" )
...
function ENT:PlayerEnterPortal(ent)
	...
	if ent.JustEntered then
		ent:EmitSound("player/portal_enter"..math.random(1,2)..".wav",80,100 + (30 * (ent:GetVelocity():Length() - 100)/1000))
		ent.JustEntered = false
	end
end

function ENT:SetupPlayerClone(ply)
	if !IsValid(ply.PortalClone) then
...
function ENT:DoPort(ent) --Shared so we can predict it.
	...
	local portal = self:GetNWEntity("Potal:Other")
	if !IsValid(portal) then return end
	...
	if IsValid(portal) and IsValid(phys) and IsValid(ent.clone) and !ent:IsPlayer() then
	...
	elseif ent:IsPlayer() then
		...
		elseif ent.InPortal == self then
			...
			if SERVER then
				ent:EmitSound("player/portal_exit"..math.random(1,2)..".wav",80,100 + (30 * (nuVel:Length() - 100)/1000))
			end
...
function ENT:AcceptInput(name, activator, caller, data) --Map inputs (Seems to work..)
...
function ENT:KeyValue( key, value ) --Map keyvalues
	...
	if key_lowcase == "linkagegroupid" then --I don't think this does jack shit, but it was on the valve wiki..
		self:SetNWInt("Potal:LinkageGroupID",value)
	end
	
	if key_lowcase == "activated" then --Set if it should start activated or not..
		self.Activated = tobool(value)
		self:SetNWBool("Potal:Activated",tobool(value))
	end
	
	if key_lowcase == "portaltwo" then --Sets the portal type
